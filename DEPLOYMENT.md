...
cp -r /Volumes/MacMiniM4-ext\ 1/Development/files_archive /path/to/nextcloud/apps/files_archive
...
```bash
sudo chown -R www-data:www-data /path/to/nextcloud/apps/files_archive
```
...
```bash
cd /path/to/nextcloud/apps/files_archive
...
sudo find /path/to/nextcloud/apps/files_archive -type d -exec chmod 755 {} \;

# Set file permissions
sudo find /path/to/nextcloud/apps/files_archive -type f -exec chmod 644 {} \;
```
...
You should see `files_archive` in the enabled apps list.
...
cp -r /path/to/files_archive /path/to/nextcloud/apps/
...
# 4. Enable app
cd /path/to/nextcloud
php occ app:enable files_archive
...
scp -r js/ user@server:/opt/stacks/nextcloud/apps/files_archive/
...
1. **Check file permissions:**
   ```bash
   ls -la /path/to/nextcloud/apps/files_archive
   ```

2. **Check appinfo/info.xml syntax:**
   ```bash
   xmllint --noout /path/to/nextcloud/apps/files_archive/appinfo/info.xml
   ```
...
1. **Backup current installation:**
   ```bash
   cp -r /path/to/nextcloud/apps/files_archive /path/to/backup/files_archive
   ```
...
# 1. Disable the app
cd /path/to/nextcloud
sudo -u www-data php occ app:disable files_archive

# 2. Remove the app directory
rm -rf /path/to/nextcloud/apps/files_archive
...
- [App Repository](https://github.com/nextcloud/files_archive)
