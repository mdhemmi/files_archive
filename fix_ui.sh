...
php occ maintenance:mode --on 2>/dev/null || echo "   (Skipping maintenance mode)"
php occ maintenance:mode --off 2>/dev/null || echo "   (Skipping maintenance mode)"
echo "   ✓ Cache cleared"
...
echo "- Nextcloud logs: tail -f /opt/stacks/nextcloud/data/nextcloud.log"
